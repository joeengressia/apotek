...
class M_Rawatinap extends CI_Model {
    ...
    function data_pasien_muat_data($q) {
        ...
        left join dinamis_penduduk dp on (dp.id = pdf.dinamis_penduduk_id)
        ...
        where pdf.waktu_keluar is NULL and p.no_rm like ('%$q%') or pd.nama like ('%$q%')
        order by locate ('$q',p.no_rm) limit 0, 15";
        //echo "<pre>".$sql."</pre>";
