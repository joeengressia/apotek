...
class M_autocomplete extends CI_Controller {
    ...
    function dokter($q) {
        $sql = "select p.*, IFNULL(pr.nama,'') as spesialis, dp.str_no, dp.sip_no 
            ...
            where p.nama like ('%$q%') and p.status = 'Dokter' order by locate('$q', p.nama)";
        return $this->db->query($sql);
    }
    ...
    function pasien($q) {
        ...
        where p.nama like ('%$q%') or ps.no_rm like ('%$q%') order by locate('$q', ps.no_rm) limit 0, 20";
        return $this->db->query($sql);
    }
    
    function pasien_active($q) {
        $sql = "select p.*, dp.alamat, ps.no_rm
        ...
        where (p.nama like ('%$q%') or ps.no_rm like ('%$q%'))
        and pdf.waktu_keluar is NULL    
        order by locate('$q', ps.no_rm) limit 0, 20";
        return $this->db->query($sql);
    }
    ...
    function get_no_resep() {
        $row = $this->db->query("select substr(id, 1, 4) as jumlah from resep where date(waktu) like '%".date("Y-m")."%' order by waktu desc limit 1")->row();
        if (!isset($row->jumlah)) {
            $str = "0001-".date("m")."/".date("Y");
        } else {
            $str = str_pad((string)($row->jumlah+1), 4, "0", STR_PAD_LEFT)."-".date("m")."/".date("Y");
        }
        return $str;
    }
    ...
    function get_detail_hpp($id_barang, $kemasan) {
        $cek = $this->db->query("select id, isi, isi_satuan from kemasan where id_barang = '$id_barang' and id_kemasan = '$kemasan'")->row();
        $row = $this->db->query("select * from detail_penerimaan where id_kemasan = '".$cek->id."' order by id desc limit 1")->row();
