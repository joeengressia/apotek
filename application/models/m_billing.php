...
class M_billing extends CI_Model {
    ...
    function penjualan_barang_load_data2($no_daftar) {
        $sql = "select p.total, p.id as no_penjualan, b.kekuatan, rp.id as id_retur, b.generik, td.disc_pr as diskon, (td.harga_jual*td.qty) as subtotal,
            p.id_resep as status, td.*, td.qty as keluar, bp.id as id_pb, (b.hna*bp.isi*bp.isi_satuan) as hna, bp.barcode, b.margin_resep as margin,
    ...
    function get_jasa_apoteker($no_daftar) {
        $sql = "select sum(nominal) as total_jasa_apt from resep_r rr
            join resep r on (rr.id_resep = r.id)
