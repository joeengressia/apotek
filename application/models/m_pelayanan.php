...
class M_pelayanan extends CI_Model {
    ...
    function pelayanan_kunjungan_get_data($id){
        $sql = "select pk.* ,jjk.nama as jenis_jurusan, jk.nama as unit_layanan,
                u.nama as nama_unit, p.nama as nama_pegawai, t.nomor as no_tt,
                ...
                left join penduduk p on (p.id = peg.penduduk_id)
                left join asuransi_produk asu on(asu.id = pk.id_produk_asuransi)
                left join tt t on (t.id = pk.no_tt)
                where pk.id = '$id' ";
        $query = $this->db->query($sql);
        return $query->row();
    }
    ...
    function tindakan_pelayanan_kunjungan_save($id_pelayanan_kunjungan){
        ...
        if (is_array($waktu)) {
             ...
             foreach ($waktu as $key => $value) {
                ...
                $cek = $this->db->query("select count(*) as jumlah from penjualan where id_pelayanan_kunjungan = '$id_pelayanan_kunjungan'")->row();
                if ($cek->jumlah === 0) {
                    $data_jual = array(
                        'id_pelayanan_kunjungan' => $id_pelayanan_kunjungan,
                    ...
                    $this->db->insert('penjualan', $data_jual);
                    $id_penjualan = $this->db->insert_id();
                }
                $bhp = $this->db->query("select b.*, (br.hna+(br.hna*(br.margin_non_resep/100))) as harga, b.id as id_barang,
                    (k.isi*k.isi_satuan) as isisatuan
                    from bhp_tarif b
                    join kemasan k on (b.id_kemasan_barang = k.id) 
                    join barang br on (k.id_barang = br.id) where b.id_tarif = '$tarif[$key]'")->result();
                foreach ($bhp as $data) {
                    $brg = $this->db->query("select sum(masuk)-sum(keluar) as sisa, ed from stok where id_barang = '".$data->id."' group by id_barang, ed having sisa > 0 order by ed asc limit 1")->row();
                    $data_detail_jual = array(
                        'id_penjualan' => $id_penjualan,
                        'id_kemasan' => $data->id_kemasan_barang,
                        'expired' => isset($brg->ed)?$brg->ed:NULL,
                        'qty' => $data->jumlah,
                        'hna' => $data->hna,
                        'harga_jual' => $data->harga
                    );
                    $this->db->insert('detail_penjualan', $data_detail_jual);
                    ...
                    $data_stok = array(
                        ...
                        'transaksi' => 'Penjualan',
                        'id_barang' => $data->id,
                        'ed' => isset($brg->ed)?$brg->ed:NULL,
                        'keluar' => $data->jumlah*$data->isisatuan,
    ...
    function delete_tindakan($id){
        $get = $this->db->query("select id_pelayanan_kunjungan as id_pk, id_tarif from tindakan_pelayanan_kunjungan where id = '$id'")->row();
        
        //$this->db->delete('jasa_pelayanan_kunjungan', array('id_pelayanan_kunjungan' => $get->id_pk, ''));
        $this->db->delete('tindakan_pelayanan_kunjungan', array('id' => $id));
    }
    ...
    function get_data_resep($limit, $start, $search) {
        ...
        $sql   = "select r.*, pj.id as id_jual, rr.id_jasa_apoteker as id_rr, rr.id_resep, rr.id_barang, rr.id_jasa_apoteker, rr.r_no, concat_ws(' ',b.nama, b.kekuatan, s.nama) as nama_barang, 
    ...
    function get_list_data_resep($id, $status, $no_r = NULL) {
        $q = NULL;
        if ($status === 'group') {
            $q = " group by rr.r_no";
        } 
        ...
        $sql = "select concat_ws(' ',b.nama, b.kekuatan, s.nama) as nama_barang,
            concat_ws(' ',rr.pakai, ' X ', rr.aturan) as aturan_pakai, rr.*
            from resep_r rr 
            join barang b on (b.id = rr.id_barang)
            left join satuan s on (b.satuan_kekuatan = s.id)
            left join jasa_apoteker t on (t.id = rr.id_jasa_apoteker)
            left join sediaan sd on (sd.id = b.id_sediaan)
            where rr.id_resep = '$id' $q";
        //echo $sql;
        return $this->db->query($sql);
    }
    
    function get_list_data_resep_detail($id, $no_r) {
        ...
        $sql = "select concat_ws(' ',b.nama, b.kekuatan, s.nama) as nama_barang,
            ...
            left join jasa_apoteker t on (t.id = rr.id_jasa_apoteker)
            left join sediaan sd on (sd.id = b.id_sediaan)
            where rr.id_resep = '$id' and rr.r_no = '$no_r'";
        //echo $sql;
        return $this->db->query($sql);
    }
    ...
    function save_resep() {
        ...
        jurnal_debet('5.1.3.1.1', 'Penjualan Resep', $hna);
        jurnal_kredit('1.1.6.1.2', 'Penjualan Resep', $hna);
    ...
    function get_data_penjualan($limit, $start, $param) {
        ...
        $sql = "select p.*, date(p.waktu) as tanggal, pd.nama as customer, pd.nama as dokter, pdd.nama as customer,
            ...
            left join pasien pl on (pl.id = r.id_pasien) 
            join penduduk pd on (r.id_penduduk_dokter = pd.id)
            join penduduk pdd on (pl.id = pdd.id)
    ...
    function get_data_penjualan_resep_id($id) {
    ...
    function get_bangsal() {
        $sql = "select * from unit order by nama asc";
        return $this->db->query($sql);
    }
    
    function get_data_ranap($param) {
        $q = NULL;
        if ($param['nama'] !== '') {
            $q.=" and pdd.nama like ('%".$param['nama']."%')";
        }
        if ($param['unit'] !== '') {
            $q.=" and u.id = '".$param['unit']."'";
        }
        $sql = "select  i.*, ps.no_rm, pdd.nama as pasien, dp.alamat, CONCAT_WS(' ',u.nama,t.kelas,t.nomor) as detail from inap_rawat_kunjungan i
            join pelayanan_kunjungan pk on (i.id_pelayanan_kunjungan = pk.id)
            join pendaftaran pdf on (pk.id_kunjungan = pdf.no_daftar)
            join pasien ps on (pdf.pasien = ps.no_rm)
            join penduduk pdd on (ps.id = pdd.id)
            left join dinamis_penduduk dp on (pdd.id = dp.penduduk_id)
            join tt t on (pk.no_tt = t.id)
            join unit u on (t.unit_id = u.id)
            inner join (
                select penduduk_id, max(id) as id_max from dinamis_penduduk group by penduduk_id
            ) dm on (dm.penduduk_id = dp.penduduk_id and dp.id = dm.id_max)
            where i.keluar_waktu is NULL $q order by i.id asc";
        return $this->db->query($sql);
    }
