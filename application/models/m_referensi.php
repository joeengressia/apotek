...
class M_referensi extends CI_Model {
    ...
    function get_produk_asuransi_data($limit, $start, $search) {
        ...
        $sort = "order by ap.nama";
        $sql = "select * from asuransi_produk ap
    ...
    function penduduk_get_data($limit = null, $start = null, $search = null) {
        ...
        if (isset($search['norm']) && ($search['norm'] != '')) {
            $q.=" and ps.no_rm like ('%" . $search['norm'] . "%')";
        }
        if (isset($search['kabupaten']) && ($search['kabupaten'] != '')) {
...
//        where p.id is not null";
    ...
    function penduduk_add_data($data) {
        ...
        if ($data['status'] === 'Pasien') {
            $this->db->insert('pasien', array('id' => $id));
        }
        return $id;
    }
    ...
    function tindakan_edit_data() {
        ...
        $id_barang = post_safe('id_barang'); //array
        $id_kemasan= post_param('id_kemasan'); // array
        $qty = post_safe('qty'); // array

        if (is_array($id_barang)) {
            foreach ($id_barang as $key => $value) {
                if ($value != '') {
                    $get = $this->db->query("select id from kemasan where id_barang = '$value' and id_kemasan = '$id_kemasan[$key]'")->row();
                    $bhp = array(
                        'id_tarif' => $id_tarif,
                        'id_kemasan_barang' => $get->id,
                        'jumlah' => $qty[$key]
                    );
                    if($id_bhp[$key] !== ''){
                        // Update
    ...
    function get_data_bhp_tarif($id_tarif){
        $sql = "select bh.*, b.nama as barang, b.id as id_barang, (b.hna+(b.hna*(b.margin_resep/100))) as harga_jual, s.id as id_satuan,
                b.margin_resep as margin, IFNULL(g.diskon,0) as diskon, b.plus_ppn as ppn_jual, b.hna, (bp.isi*bp.isi_satuan) as jml_isi,
                s.nama as kemasan,
                null as harga_jual
                from bhp_tarif bh
                join kemasan bp on (bh.id_kemasan_barang = bp.id)
                join barang b on (b.id = bp.id_barang)
                join satuan s on (bp.id_kemasan = s.id)
    ...
    function get_data_supplier($limit = NULL, $start = NULL, $search = NULL) {
        $q = null; $order = NULL; $limitation = NULL;
        if ($limit !== NULL) {
        $limitation = null;
            $order = " order by nama";
        ...
        if ($search['id'] !== '' and $search['id'] !== 'undefined') {
            $q.=" and id = '".$search['id']."'";
        }
        $sql = "select * from supplier where id is not null $q $order";
       // echo $sql;
        $query = $this->db->query($sql.$limitation);
        $data['data'] = $query->result();
        $data['jumlah'] = $this->db->query($sql)->num_rows();
    ...
    function get_data_pabrik($limit = NULL, $start = NULL, $search = NULL) {
        $q = null; $order = NULL; $limitation = NULL;
        if ($limit !== NULL) {
        $limitation = null;
            $order = " order by nama";
        ...
        $sql = "select * from pabrik where id is not null $q $order";
       // echo $sql;
        $query = $this->db->query($sql.$limitation);
        $data['data'] = $query->result();
        $data['jumlah'] = $this->db->query($sql)->num_rows();
    ...
    function save_pabrik() {
        ...
        die(json_encode(array('status' => TRUE, 'id_pabrik' => $id_sup, 'nama' => $nama)));
    }
    
    function delete_pabrik($id) {
        $this->db->delete('pabrik', array('id' => $id));
    }
    
    function jasa_apoteker_load_data($limit, $start, $search) {
        $q = NULL;
        $limit = " limit $start , $limit";
        if (isset($search['nama']) && ($search['nama'] != '')) {
            $q .= " and nama like '%" . $search['nama'] . "%' ";
        }
        $sql = "select * from jasa_apoteker where id is not NULL $q";
        //echo $sql;
    ...
    function jasa_apoteker_save() {
        $id     = post_param('id');
        $nama   = post_param('jasa_apoteker');
        $nominal= currencyToNumber(post_param('nominal'));
        ...
        if ($id === '') {
            $this->db->insert('jasa_apoteker', $data);
            $id_jasa = $this->db->insert_id();
        } else {
            $this->db->where('id', $id);
            $this->db->update('jasa_apoteker', $data);
            $id_jasa = $id;
        }
        return array('status' => TRUE, 'id' => $id_jasa);
    }
    
    function jasa_apoteker_delete($id) {
        $this->db->delete('jasa_apoteker', array('id' => $id));
    }
}
?>