...
class M_inventory extends CI_Model {
    ...
    function inkaso_load_data($limit, $start, $param) {
        $q = null;
        if ($param['key'] !== '') {
            $q.=" having p.faktur like ('%".$param['key']."%')";
        }
        $sql = "select i.*, p.faktur, r.nama as supplier, IFNULL(b.nama,'-') as bank
            from inkaso i
            join penerimaan p on (i.id_penerimaan = p.id)
            join supplier r on (p.id_supplier = r.id)
            left join bank b on (i.id_bank = b.id)
            where i.id is not NULL $q order by p.id, i.tanggal
            desc";
...
//echo $sql;
    ...
    function load_data_probabilitas($param) {
        $q = NULL;
        if ($param['awal'] !== '') {
        ...
        $result['list_data'] = $this->db->query($sql)->result();
        $result['total']= $this->db->query($sql_jml)->num_rows();
        return $result;
    }
    
    function load_data_abc($param) {
        $q = NULL;
        if ($param['awal'] !== '') {
            $q.=" and date(p.waktu) between '".$param['awal']."' and '".$param['akhir']."'";
        }
        ...
        $sql_jml = "select sum(b.hna+(b.hna*(b.margin_non_resep/100))) as total
        from penjualan p
        join detail_penjualan dp on (p.id = dp.id_penjualan)
        join kemasan k on (k.id = dp.id_kemasan)
        join barang b on (k.id_barang = b.id)
        ...
        //echo $sql_jml;
        $result['list_data'] = $this->db->query($sql)->result();
        $result['total']= $this->db->query($sql_jml)->row();
        return $result;
    ...
    function get_data_pemesanan($limit, $start, $search) {
        ...
        if (isset($search['awal'])) {
            $q.=" and date(p.tanggal) between '".$search['awal']."' and '".$search['akhir']."'";
        }
        if (isset($search['id_supplier']) and $search['id_supplier'] !== '') {
            $q.=" and p.id_supplier = '".$search['id_supplier']."'";
        }
        $sql = "select p.*, k.nama as karyawan, dp.jumlah, concat_ws(' ',b.nama, b.kekuatan, st.nama) as nama_barang, b.perundangan,
    ...
    function get_data_stok_opname($limit, $start, $search) {
        ...
        $sql = "select s.*, b.kekuatan, b.nama, st.nama as satuan_kekuatan, sum(s.masuk) as masuk, sum(s.keluar) as keluar, 
            ...
            join barang b on (s.id_barang = b.id)
            left join satuan st on (b.satuan_kekuatan = st.id)
            where s.transaksi = 'Stok Opname'  and s.id_unit = '".$this->session->userdata('id_unit')."' $q group by s.id_barang";
        $query = $this->db->query($sql . $limitation);
        //echo $sql . $q . $limitation;
        $queryAll = $this->db->query($sql);
        $data['data'] = $query->result();
    ...
    function save_penerimaan() {
        ...
        if ($id_penerimaan === '') {
            ...
            if ($materai !== '0') {
                jurnal_debet('5.2.3.1.2', 'Penerimaan', $materai);
                jurnal_kredit('1.1.6.2.2', 'Penerimaan', $materai);
            }
            if ($this->db->trans_status() === FALSE) {
                $this->db->trans_rollback();
                $result['status'] = FALSE;
    ...
    function save_pemusnahan() {
        $this->db->trans_begin();
        $tanggal    = date2mysql(post_param('tanggal'));
        $saksi_rs   = post_param('id_apoteker');
        $saksi_bpom = post_param('bpom');
        $data_pemusnahan = array(
            'tanggal' => $tanggal,
            'saksi_apotek' => $saksi_rs,
            'saksi_bpom' => $saksi_bpom
        );
        $this->db->insert('pemusnahan', $data_pemusnahan);
        $id_pemusnahan = $this->db->insert_id();
        if ($this->db->trans_status() === FALSE) {
        ...
        foreach ($id_barang as $key => $data) {
            if ($data !== '') {
                $brg = $this->db->query("select id from kemasan where id_barang = '$data' and id_kemasan = '$id_kemasan[$key]'")->row();
                $data_detail = array(
                ...
                $this->db->insert('detail_pemusnahan', $data_detail);
                if ($this->db->trans_status() === FALSE) {
                    $this->db->trans_rollback();
                    $result['status'] = FALSE;
                }
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $this->db->trans_commit();
            $result['status'] = TRUE;
            $result['id'] = $id_pemusnahan;
        }
        return $result;
    }
    
    function delete_pemusnahan($id) {
        $this->db->delete('pemusnahan', array('id' => $id));
    }
    ...
    function stok_load($param) {
        $q = NULL; $select = NULL;
        if ($param['awal'] !== '' and $param['akhir'] !== '') {
            $q.=" and date(s.waktu) between '".$param['awal']."' and '".$param['akhir']."'";
        }
        if ($param['barang'] !== '') {
            $q.=" and b.id = '".$param['barang']."'";
        }
        if ($param['jenis'] === 'History') {
            $q.=" order by nama_barang";
        }
        if ($param['jenis'] === 'Sisa') {
            $select = "sum(s.masuk)-sum(s.keluar) as sisa, sum(s.masuk) as masuk, sum(s.keluar) as keluar,";
            $q.=" group by s.id_barang, s.ed";
        }
        $sql = "select s.*, $select CONCAT_WS(' ',b.nama, b.kekuatan, st.nama) as nama_barang from stok s
            join barang b on (s.id_barang = b.id)
            left join satuan st on (b.satuan_kekuatan = st.id)
            where s.id is not NULL and s.id_unit = '".$this->session->userdata('id_unit')."' $q";
        //echo $sql;
        return $this->db->query($sql);
    }
    
    function koreksi_stok_load($limit, $start, $param) {
        $q = NULL;
        if ($param['key'] !== '') {
            $q.=" having nama_barang like ('%".$param['key']."%')";
        }
        $sql = "select s.*, sum(masuk) as masuk, sum(keluar) as keluar, sum(s.masuk)-sum(s.keluar) as sisa, CONCAT_WS(' ',b.nama, b.kekuatan, st.nama) as nama_barang from stok s
            join barang b on (s.id_barang = b.id)
            left join satuan st on (b.satuan_kekuatan = st.id)
            where s.id is not NULL group by s.id_barang, s.ed 
            and s.id_unit = '".$this->session->userdata('id_unit')."' $q order by nama_barang";
    ...
    function save_koreksi_stok() {
        $kenyataan  = post_param('kenyataan');
        $penyesuaian= post_param('penyesuaian');
        $alasan     = post_param('alasan');
        $id_barang  = post_param('id_barang');
        $ed         = post_param('ed');
        
        foreach ($kenyataan as $key => $data) {
            if ($data !== '') {
                if ($penyesuaian[$key] >= 0) {
                    $array = array(
                        'waktu' => date("Y-m-d H:i:s"),
    ...
    /*RETUR PENERIMAAN*/
    function get_data_retur_penerimaan($limit, $start, $search) {
        $q = NULL; $limitation = NULL;
        if (isset($search['id']) and $search['id'] !== '') {
            $q.=" and rp.id = '".$search['id']."' ";
        }
        if (isset($search['id_supplier'])) {
            //$q.=" and p.status != 'Cash'";
        }
        ...
        $sql = "select rp.id as id_retur_penerimaan, rp.tanggal, rp.id_supplier, k.isi, k.id_kemasan as id_satuan, b.id as id_barang, st.nama as kemasan, 
        b.nama as barang, b.kekuatan, stn.nama as satuan, dp.*, s.nama as supplier
        from retur_penerimaan rp
    ...
    function save_retur_penerimaan() {
        $this->db->trans_begin();
        $tanggal        = date2mysql(post_param('tanggal'));
        $id_supplier    = post_param('id_supplier');
        $id_barang      = post_param('id_barang');
        $id_kemasan     = post_param('id_kemasan');
        $ed             = post_param('ed');
        $jumlah         = post_param('jumlah');
        $jml_isi        = post_param('jml_isi');
        $id_retur       = post_param('id_retur_penerimaan');
        ...
        if ($id_retur === '') {
            ...
            foreach ($id_barang as $key => $data) {
                $kemasan = $this->db->query("select id from kemasan where id_barang = '$data' and id_kemasan = '$id_kemasan[$key]'")->row();
                ...
                $row = $this->db->query("select dp.* from kemasan k
                    join barang b on (k.id_barang = b.id) 
                    join detail_penerimaan dp on (k.id = dp.id_kemasan)
                    where b.id = '$data' and k.id = '$id_kemasan[$key]' and dp.expired = '".date2mysql($ed[$key])."'")->row();
            ...
            $sql = "update retur_penerimaan set
                tanggal = '$tanggal',
                id_supplier = '$id_supplier',
                id_users = '".$this->session->userdata('id_user')."'
                where id = '$id_retur'";
            ...
            $this->db->delete('detail_retur_penerimaan', array('id_retur_penerimaan' => $id));
            $this->db->delete('stok', array('id_transaksi' => $id, 'transaksi' => 'Retur Penerimaan'));
            if ($this->db->trans_status() === FALSE) {
                $this->db->trans_rollback();
                $result['status'] = FALSE;
            }
    ...
    function delete_retur_penerimaan($id) {
        $this->db->delete('retur_penerimaan', array('id' => $id));
        $this->db->delete('stok', array('id_transaksi' => $id, 'transaksi' => 'Retur Penerimaan'));
    }
    
    function get_distributor_by_barang($id_barang) {
        $sql = "select s.nama from supplier s
    ...
    function add_rencana_pemesanan($id) {
        $this->db->query("insert INTO defecta set
            id_barang = '$id',
            jumlah = '1'");
    }
    
    function get_data_faktur($q) {
        $sql = "select p.*, s.nama as supplier, sum(i.nominal) as terbayar from penerimaan p
            join supplier s on (p.id_supplier = s.id) 
    ...
    function get_sisa_hutang_supplier($faktur) {
        $hutang = $this->db->query("select sum(total) as total_hutang FROM penerimaan where faktur = '$faktur'")->row();
        $terbyr= $this->db->query("select sum(i.nominal) as terbayar from inkaso i join penerimaan p on (i.id_penerimaan = p.id) where p.faktur = '$faktur'")->row();
