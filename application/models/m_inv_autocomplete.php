...
class M_inv_autocomplete extends CI_Model {
    ...
    function load_data_packing_barang($q, $extra_param = null) {
        ...
        $sql = "select b.*, k.id as id_kemasan, CONCAT_WS(' ', b.nama, b.kekuatan, s.nama) as nama_barang
                from barang b
                join kemasan k on (b.id = k.id_barang)
                join satuan s on (s.id = b.satuan_kekuatan)
                join satuan st on (st.id = k.id_kemasan)
                where b.nama like ('%$q%') $param order by k.isi desc";
        //echo $sql;
        return $this->db->query($sql);
    }
    ...
    function get_harga_barang_penjualan($id, $id_kemasan) {
        $sql = "select (b.hna+(b.hna*(b.margin_non_resep/100))) as harga, g.diskon, b.plus_ppn as ppn_jual, (k.isi*k.isi_satuan) as jumlah
            from barang b 
            join kemasan k on (b.id = k.id_barang) 
