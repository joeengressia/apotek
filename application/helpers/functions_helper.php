...
function jurnal_debet($kode_rekening, $nama_transaksi, $debet, $id_transaksi = NULL, $keterangan = NULL, $id_rekening = NULL) {
    $CI = & get_instance();
    if ($id_rekening !== NULL) {
        $id_rek = $id_rekening;
    } else {
        $get= $CI->db->query("select id from rekening where kode = '$kode_rekening'")->row();
        $id_rek = $get->id;
    }
    $data_jurnal = array(
    ...
    $CI->db->insert('jurnal', $data_jurnal);
    if ($CI->db->trans_status() === FALSE) {
        $CI->db->trans_rollback();
    }
}

function jurnal_kredit($kode_rekening, $nama_transaksi, $kredit, $id_transaksi = NULL, $keterangan = NULL, $id_rekening = NULL) {
    $CI = & get_instance();
    if ($id_rekening !== NULL) {
        $id_rek = $id_rekening;
    } else {
