...
class Laporan extends CI_Controller {
    ...
    function cetak_jasa_klinis() {
        
        $param['nakes'] = get_param('id');
        $param['awal'] = get_safe('awal');
        $param['akhir'] = get_safe('akhir');
        $data['list_data'] = $this->m_billing->penjualan_jasa_load_data($param)->result();
        $data['apt'] = $this->configuration->rumah_sakit_get_atribute()->row();
        $this->load->view('laporan/penjualan-jasa-print', $data);
    }
...
//        $data['rawat_inap'] = $this->m_billing->rawat_inap_detail_load_data_detail($id)->result();
    ...
    function rekap_resep_load() {
        ...
        $this->load->view('laporan/rekap-resep-list', $data);
    }
    ...
    function neraca_load_data() {
        ...
        $data['pendapatan_operasional'] = $this->m_akuntansi->neraca_load_data('pendapatan')->result();
        $data['beban_operasional'] = $this->m_akuntansi->neraca_load_data('beban')->result();
        $this->load->view('akuntansi/neraca-table', $data);
    }
    ...
    function load_data_abc() {
        $param['awal']  = date2mysql(get_safe('awal'));
        $param['akhir'] = date2mysql(get_safe('akhir'));
        $data = $this->m_inventory->load_data_abc($param);
    ...
    function load_data_lap_probabilitas() {
        $param['awal']  = date2mysql(get_safe('awal'));
        $param['akhir'] = date2mysql(get_safe('akhir'));
        $data = $this->m_inventory->load_data_probabilitas($param);
