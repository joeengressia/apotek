...
class Referensi extends CI_Controller {
    ...
    function penduduk() {
        ...
        $data['title'] = 'Kependudukan (Pasien, Dokter, Apoteker)';
    ...
    function jasa_apoteker(){
        $data['title'] = 'Jasa Apoteker';
        $this->load->view('referensi/layanan/jasa-apoteker', $data);
    }

    function get_jasa_apoteker_list($param, $page) {
        $limit = 10;
        if ($page == 'undefined') {
            $page = 1;
        }
        ...
        $data['page'] = $page;
        $data['limit'] = $limit;
        $query = $this->m_referensi->jasa_apoteker_load_data($limit, $start, $param);
        $data['jumlah'] = $query['jumlah'];
        $data['list_data'] = $query['data'];
        $data['paging'] = paging_ajax($data['jumlah'], $limit, $page, 1, '');
        return $data;
    ...
    function manage_jasa_apoteker($mode, $page = null) {
        $param = array();
        switch ($mode) {
            case 'list': 
                if(get_safe('nama') != ''){
                ...
                $data = $this->get_jasa_apoteker_list($param, $page); 
                $data['nama'] = get_safe('nama');
                $this->load->view('referensi/layanan/jasa-apoteker-list', $data);
            break;
            case 'save':
                $data = $this->m_referensi->jasa_apoteker_save();
                die(json_encode($data));
            break;
            case 'delete':
                $id = get_safe('id');
                $this->m_referensi->jasa_apoteker_delete($id);
            break;
            default:
                break;
        }
    ...
    function pabrik() {
        ...
        $this->load->view('referensi/pabrik/pabrik', $data);
    }
    ...
    function manage_pabrik($status, $page = null) {
        ...
        switch ($status) {
            case 'list':
                ...
                $this->load->view('referensi/pabrik/pabrik-list', $data);
                break;
            case 'save': 
                $data = $this->m_referensi->save_pabrik();
                die(json_encode($data));
                break;
            case 'delete': 
    ...
    function supplier() {
        ...
        $this->load->view('referensi/supplier/supplier', $data);
    }
    ...
    function manage_supplier($status, $page = null) {
        ...
        switch ($status) {
            case 'list':
                ...
                $this->load->view('referensi/supplier/supplier-list', $data);
                break;
            case 'save': 
                $data = $this->m_referensi->save_supplier();
                die(json_encode($data));
                break;
            case 'delete': 
