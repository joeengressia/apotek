...
class Autocomplete extends CI_Controller {
    ...
    function pasien_active() {
        $q = $_GET['q'];
        $data = $this->m_autocomplete->pasien_active($q)->result();
        die(json_encode($data));
    }
...
//    }
    ...
    function get_data_retur_penerimaan($id) {
        $search['id'] = $id;
        $data = $this->m_inventory->get_data_retur_penerimaan(NULL, NULL, $search);
        die(json_encode($data));
    }
    
    function faktur() {
        $q = get_param('q');
        $data = $this->m_inventory->get_data_faktur($q)->result();
        die(json_encode($data));
    }
    
    function get_sisa_hutang_supplier() {
        $data = $this->m_inventory->get_sisa_hutang_supplier($_GET['faktur']);
        die(json_encode(array('sisa' => $data)));
    }
    
    function get_detail_hpp() {
        $data = $this->m_autocomplete->get_detail_hpp(get_param('id'), get_param('id_kemasan'));
        die(json_encode(array('total_hpp' => $data)));
    }
}
?>
