...
class Pelayanan extends CI_Controller {
    ...
    function manage_resep($status, $page = null) {
        ...
        switch ($status) {
            ...
            case 'cetak_copy':
                ...
                $data['data'] = $this->get_list_data_resep($limit, $page, $param);
                $data['detail'] = $this->m_pelayanan->get_list_data_resep(get_param('id'), 'group')->result();
    ...
    function manage_penjualan($status, $page = null) {
        ...
        switch ($status) {
            ...
            case 'cetak': 
                $search['id']  = $_GET['id'];
                $data['arr'] = $this->m_pelayanan->get_data_penjualan_resep_id($_GET['id']);
                $data['apt'] = $this->configuration->rumah_sakit_get_atribute()->row();
                $this->load->view('pelayanan/penjualan-non-resep-print', $data);
                break;
    ...
    function manage_laporan_penjualan($status, $page = null) {
        $limit = 15;
        switch ($status) {
            case 'listresep':
                $search['id']  = '';
                ...
                $search['id_dokter']= get_param('id_dokter');
                $search['id_pasien']= get_param('id_pasien');
                $search['laporan']  = 'detail';
                $data = $this->get_list_data_penjualan($limit, $page, $search);
                $this->load->view('laporan/lap-penjualan-resep-list', $data);
                break;
            case 'listnonresep': 
                $search['id']  = '';
                $search['awal']     = date2mysql(urldecode(get_param('awal')));
                ...
                $this->load->view('laporan/lap-penjualan-nonresep-list', $data);
                break;
    ...
    function manage_penjualan_non_resep($status, $page = null) {
        ...
        switch ($status) {
            ...
            case 'cetak': 
                $search['id']  = $_GET['id'];
                $data['arr'] = $this->m_pelayanan->get_data_penjualan_non_resep(NULL, NULL, $search);
                $data['apt'] = $this->configuration->rumah_sakit_get_atribute()->row();
                $this->load->view('pelayanan/penjualan-non-resep-print', $data);
                break;
    ...
    function informasi_ranap() {
        $data['title'] = 'Informasi Pasien Ranap';
        $data['list_bangsal'] = $this->m_pelayanan->get_bangsal()->result();
        $this->load->view('pelayanan/info-ranap', $data);
    }
    
    function get_data_ranap() {
        $param['nama'] = post_param('nama');
        $param['unit'] = post_param('bangsal');
        $data['data_list'] = $this->m_pelayanan->get_data_ranap($param)->result();
        $this->load->view('pelayanan/info-ranap-list', $data);
    }
}
?>