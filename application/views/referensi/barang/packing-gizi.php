<?php $this->load->view('message') ?>
    <script type="text/javascript">
        ...
        $(function(){
            $('#barang').focus();
            $('#simpan-gizi').button({icons: {secondary: 'ui-icon-circle-check'}}).click(function(){
                form_submit();
            });
            $('#cari-gizi').button({icons: {secondary: 'ui-icon-search'}});
            $('#reset-gizi, #cetak_batal').button({icons: {secondary: 'ui-icon-refresh'}});
            $('#cetak-jumlah').button({icons: {secondary: 'ui-icon-print'}});
            get_packing_list(1, '');
            $('#reset-gizi').click(function(){
                load_my_page('<?= base_url('referensi/packing_barang_gizi') ?>','#tabs-2');
            });
            ...
            $('#form_cari_packing').submit(function(){
                form_cari_submit();
                return false;
            });
        ...
        function form_submit(){
            ...
            if($('input[name=id_barang]').val()==''){
                ...
                $.ajax({
                    url: '<?= base_url('referensi/manage_packing') ?>/cek/'+null+'/Gizi/',
        ...
        function save(){
            ...
            if(tip == ''){
                Url = '<?= base_url('referensi/manage_packing') ?>/add/'+null+'/Gizi/';
            }else{
                Url = '<?= base_url('referensi/manage_packing') ?>/edit/'+null+'/Gizi/';
            }
        ...
        function get_packing_list(p, search){
            $.ajax({
                type : 'GET',
                url: '<?= base_url('referensi/manage_packing') ?>/list/'+p+'/Gizi/',
                data: $('#formpacking').serialize(),
                cache: false,
                success: function(data) {
                    $('#packing_list').html(data);
                 ...
                 buttons: [ 
                    { 
                        ...
                        click: function() { 
                            $.ajax({
                                type : 'GET',
                                url: '<?= base_url('referensi/manage_packing') ?>/delete/'+$('.noblock').html()+'/Gizi/',
    ...
    <div class="data-input">
        <fieldset>
            <tr><td>Barcode</td><td><?= form_input('barcode', get_last_id('barang_packing','id'), 'id=barcode size=40') ?>
            <?= form_open('', 'id=formpacking') ?>
            
            <?= form_hidden('id') ?>


            <tr><td>Barang</td><td><?= form_hidden('barcode', '', 'size=40') ?>
            ...
            <tr><td>Isi @</td><td><?= form_input('isi', '', 'id=isi size=40 onkeyup=Angka(this)') ?>
            <tr><td>Satuan</td><td><?= form_dropdown('satuan', $satuan, null, 'id=satuan') ?>
            <tr><td></td><td><?= form_button('', 'Simpan', 'id=simpan-gizi') ?>
            <?= form_button('','Cari','id=cari-gizi onclick=get_packing_list(1)') ?>
            <?= form_button(null, 'Reset', 'id=reset-gizi') ?>
