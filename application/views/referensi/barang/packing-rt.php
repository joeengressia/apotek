...
<script src="<?= base_url() ?>assets/js/jquery-barcode-2.0.2.min.js"></script>
    ...
    <script type="text/javascript">
        ...
        $(function(){
            ...
            $('#reset-rt, #cetak_batal').button({icons: {secondary: 'ui-icon-refresh'}}); 
            $('#cari-rt').button({icons: {secondary: 'ui-icon-search'}});
            $('#cetak-jumlah').button({icons: {secondary: 'ui-icon-print'}});
            get_packing_list(1, '');
            $('#reset-rt').click(function(){
                load_my_page('<?= base_url('referensi/packing_barang_rt') ?>','#tabs-2');
            });
        ...
        function form_submit(){
            ...
            if($('input[name=id_barang]').val()==''){
                ...
                $.ajax({
                    url: '<?= base_url("referensi/manage_packing") ?>/cek/'+null+'/Rumah Tangga/',
        ...
        function save(){
            ...
            if(tip == ''){
                Url = '<?= base_url('referensi/manage_packing') ?>/add/'+null+'/Rumah Tangga/';
            }else{
                Url = '<?= base_url('referensi/manage_packing') ?>/edit/'+null+'/Rumah Tangga/';
            }
        ...
        function get_packing_list(p, search){
            $.ajax({
                type : 'GET',
                url: '<?= base_url('referensi/manage_packing') ?>/list/'+p+'/Rumah Tangga/',
                data: $('#formpacking').serialize(),
                cache: false,
                success: function(data) {
                    $('#packing_list').html(data);
                 ...
                 buttons: [ 
                    { 
                        ...
                        click: function() { 
                            $.ajax({
                                type : 'GET',
                                url: '<?= base_url('referensi/manage_packing') ?>/delete/'+$('.noblock').html()+'/Rumah Tangga/',
    ...
    <div class="data-input">
        ...
        <tr><td></td><td><?= form_submit('', 'Simpan', 'id=simpan') ?>
            <?= form_button('','Cari','id=cari-rt onclick=get_packing_list(1)') ?>
            <?= form_button(null, 'Reset', 'id=reset-rt') ?>
        <?= form_close() ?>
        </table>
    </div>
