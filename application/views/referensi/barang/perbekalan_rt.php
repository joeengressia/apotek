...
function form_submit(){
    ...
    if(nama_brg ==''){
        ...
        $.ajax({
            url: '<?= base_url('referensi/manage_barang_perbekalan_rt') ?>/cek',
            data:'nama='+nama_brg,
            cache: false,
            dataType: 'json',
...
function save_barang_non(){
    ...
    if( tipe== ''){
        Url = '<?= base_url('referensi/manage_barang_perbekalan_rt') ?>/add/';
    }else{
        Url = '<?= base_url('referensi/manage_barang_perbekalan_rt') ?>/edit/';
    }
    ...
    if(!request) {
        request =  $.ajax({
            ...
            success: function(data) {
                ...
                $.ajax({
                    type : 'GET',
                    url: '<?= base_url('referensi/manage_barang_perbekalan_rt') ?>/get_data/1',
                    data :'id='+id,
                    cache: false,
                    success: function(data) {
                        $('#non_list').html(data);
...
function get_nonobat_list(p,search){
    $.ajax({
        type : 'GET',
        url: '<?= base_url('referensi/manage_barang_perbekalan_rt') ?>/list/'+p,
        data : $('#formnon').serialize(),
        cache: false,
        success: function(data) {
            $('#non_list').html(data);
         ...
         buttons: [ 
            { 
                ...
                click: function() { 
                    $.ajax({
                        type : 'GET',
                        url: '<?= base_url('referensi/manage_barang_perbekalan_rt') ?>/delete/'+$('.noblock').html(),
                        data :'id='+id,
                        cache: false,
                        success: function(data) {
...
<div class="data-input">
    <table width="100%" class="inputan">Parameter</legend>
        ...
        <tr><td>Kategori</td><td><?= form_dropdown('kategori_rt', $kategori_rt, null, 'id=kategori') ?>
        <tr><td>Pabrik</td><td><?= form_input('pabrikbarang', '', 'class=pabrik size=40') ?>
