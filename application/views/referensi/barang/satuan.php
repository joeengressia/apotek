...
<script type="text/javascript">
    $(function() {
        ...
        $('#formsatuan').submit(function() {
            var url = $(this).attr('action');
            var tipe = $('input[name=id_satuan]').val();
            if(tipe == ''){
                url += '/add/1';
            }else{
                url += '/edit/1';
            ...
            $.ajax({
                ...
                success: function(data) {
                    var view = data.split('##');
                    $('input[name=id_satuan]').val($.trim(view[0]));
                    $('#list').html(view[1]);
                    if(tipe == ''){
                        alert_tambah();
                    }else{
    ...
    function get_data(page){
        $.ajax({
            type: 'GET',
            url: '<?= base_url('referensi/manage_satuan') ?>/list/'+page,
            data: $('#formsatuan').serialize(),
            success: function(data) {
                $('#list').html(data);
            }
    ...
    function edit_satuan(id, nama, jenis){
        $('input[name=id_satuan]').val(id);
        $('#nama').val(nama);
    }
             ...
             buttons: [ 
                { 
                    ...
                    click: function() { 
                       $.ajax({
                            type : 'GET',
                            url: '<?= base_url('referensi/manage_satuan') ?>/delete/'+page,
                            data :'id='+id,
                            cache: false,
                            success: function(data) {
                                get_data(page);
    ...
    <div id="tabs">
        <ul>
            <li><a href="#tabs-1">Satuan</a></li>
        </ul>
        <div id="tabs-1">
            <?= form_open('referensi/manage_satuan', 'id=formsatuan') ?>
            <?= form_hidden('id_satuan') ?>
            <table class="inputan" width="100%">
                <tr><td>ID.:</td><td id="id"><?= get_last_id('satuan', 'id')?></td></tr>
