...
<div class="kegiatan">
    ...
    <script type="text/javascript">
        ...
        function hitung_pembulatan() {
            ...
            var total  = pembulatan_seribu(currencyToNumber($('#total-pembayaran').html()));
            var asuransi = 0;
            if (jumlah >= 0) {
                for (i = 0; i <= jumlah; i++) {
            ...
            $('#bulat').html(numberToCurrency(pembulatan_seribu(hasil)));
            ...
            $.getJSON('<?= base_url('billing/load_data_asuransi_by_nodaftar') ?>/'+no_daftar, function(data){
                $.each(data, function (i, val) {
                    ...
                    var str = 
                        ...
                        '<td id=total_tagihan'+i+' align=center>'+numberToCurrency(total_running)+'</td>'+
                        '<td id=pembulatan_total'+i+' align=center>'+numberToCurrency(pembulatan_seribu(total_running))+'</td>'+
                        ...
                        '<td id=sisa_tagihan'+i+' align=center></td>'+
                    ...
                    var nilai = 0;
                    if (re_pr !== '0') {
                        var nilai = Math.ceil(pembulatan*(re_pr/100));
                    }
                    if (re_rp !== '0') {
                        var nilai = re_rp;
                    }
                ...
                if (jumlah === 0) {
                    $('#asuransi').html(''); 
                    $('#asuransi').removeClass('list-data');
                }
            });
        }
        ...
        function fill_field(data){
            ...
            $.ajax({
                ...
                success: function(msg) {
                    ...
                    $('#total-pembayaran').html(numberToCurrency(pembulatan_seribu(fuck)));
                    $('#bayar').val(numberToCurrency(pembulatan_seribu(fuck)));
                    $('#total_biaya').html(numberToCurrency(pembulatan_seribu(fuck)));
                    $('#bulat').html(numberToCurrency(pembulatan_seribu(fuck)));
                    $('input[name=totallica]').val(pembulatan_seribu(fuck));
        ...
        <?= form_open('billing/pembayaran_save', 'id=formbayar') ?>
            <table width="100%" class="inputan">
                ...
                <tr><td></td><td>
                        <table width="100%" id="asuransi"></table>
                        <!--<span id="addasuransi" class="link_button" style="margin-top: 5px; cursor: pointer;"><u>Tambah Asuransi</u></span> Sementar Hide Dulu Ya-->
                </td></tr>
                <tr><td>Total Bayar Asuransi:</td><td><span class="label" id="sum_re"></span><input type="hidden" name="sumre" id="sumre" /></td></tr>
                <tr><td>Total Biaya:</td><td id="total_biaya"><?= isset($sisa)?inttocur(pembulatan_seribu($sisa)):null ?></td></tr>
                <tr><td id="label_bayar">Bayar (Rp.):</td><td><?= form_input('bayar', isset($sisa)?inttocur(pembulatan_seribu($sisa)):null, 'id=bayar onblur=FormNum(this) size=40') ?></td></tr>
                ...
                <tr><td></td><td><?= form_hidden('totallica', isset($sisa)?pembulatan_seribu($sisa):null) ?> 
                <?= isset($attribute) ? '' : form_button('data', 'Simpan', 'id=click') ?> 
