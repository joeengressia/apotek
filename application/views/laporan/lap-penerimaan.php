...
$(function() {
    ...
    $('#cetak').button({
        ...
        window.open('<?= base_url('inventory/manage_laporan_penerimaan/cetak') ?>?awal='+awal+'&akhir='+akhir+'&id_supplier='+supplier+'&faktur='+faktur, 'Penerimaan', 'width='+dWidth+', height='+dHeight+', left='+x+',top='+y);
    });
...
function get_result_penerimaan(page) {
    ...
    $.ajax({
        url: '<?= base_url('inventory/manage_laporan_penerimaan/list') ?>/'+pg,
        data: $('#form').serialize(),
        cache: false,
        type: 'GET',
