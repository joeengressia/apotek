...
$(function() {
    ...
    $('#cari').button({
        ...
        $.ajax({
            url: '<?= base_url('laporan/perinatori_load_data') ?>',
            type: 'GET',
    ...
    $('#cetak').button({
        ...
        location.href='<?= base_url('laporan/perinatori_load_data') ?>?'+$('#kegiatan_rujukan').serialize()+'&do=cetak';
    });
    $('#reset').button({
        icons: {
        ...
        $('#loaddata').empty().load('<?= base_url('laporan/rekap_perinatologi') ?>');
    });
});
</script>
<div class="kegiatan">
    ...
    <div class="data-input">
        <?= form_open('laporan/perinatori_load_data', 'id=form_rekap_perinatori') ?>
