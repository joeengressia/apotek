...
function add_new_rows(id_brg, nama_brg, jumlah, id_kemasan) {
    ...
    var str = '<tr class="tr_rows">'+
                ...
                '<td align=center><input type=hidden id=perundangan'+jml+' /><img onclick=removeMe(this); title="Klik untuk hapus" src="<?= base_url('assets/images/delete.png') ?>" align=left /></td>'+
              '</tr>';
    ...
    $.getJSON('<?= base_url('autocomplete/get_expiry_barang')?>?id='+id_brg, function(data){
        ...
        if (jmled === 0) {
            $('#ed'+jml).append("<option value=''>Pilih ...</option>");
        }
    });
    $.ajax({
        url: '<?= base_url('autocomplete/get_detail_hpp')?>?id='+id_brg+'&id_kemasan='+id_kemasan,
        dataType: 'json',
        cache: false,
        success: function(data) {
...
function form_add() {
    ...
    $('#save_pemusnahan').submit(function() {
        ...
        $.ajax({
            url: '<?= base_url('inventory/manage_pemusnahan/save') ?>',
            data: $(this).serialize(),
            dataType: 'json',
            type: 'POST',
            success: function(data) {
...
<div class="kegiatan">
    <div id="tabs">
        <ul>
            <li><a href="#tabs-1">Pemusnahan</a></li>
        </ul>
        <div id="tabs-1">
