...
<tbody>
    ...
    foreach ($list_data as $key => $data) { 
        $str = $data->id.'#'.$data->id_penduduk_dokter.'#'.$data->dokter.'#'.$data->id_pasien.'#'.$data->pasien.'#'.$data->keterangan;
        ?>
    <tr class="<?= ($data->id_resep !== $id_resep)?'odd':'even' ?>">
        ...
        <td><?= ($data->id_resep !== $id_resep)?$data->apoteker:NULL ?></td>
        ...
        <td class='aksi' align='center'>
            ...
            if ($data->id_resep !== $id_resep) { ?>
                ...
                <a class='printing' onclick="cetak_struk('<?= $data->id_jual ?>');" title="Klik untuk cetak struk">&nbsp;</a>
