    ...
    var str = '<div id=form_resep>'+
                ...
                '<table width=100% class=data-input><tr valign=top><td width=33% style="border-right: 1px solid #ccc;">'+
                    '<table width=100% cellpadding=0 cellspacing=0>'+
                        '<tr><td width=25%>Nomor Resep:</td><td><?= form_input('noresep', NULL, 'id=noresep size=10 readonly') ?></td></tr>'+
                        '<tr><td>Waktu:</td><td><?= form_input('waktu', date("d/m/Y"), 'id=waktu size=10 readonly') ?></td></tr>'+
                    ...
                    '<table align=right width=100% cellpadding=0 cellspacing=0>'+
                        ...
                        '<tr><td>TOTAL:</td><td style="font-size: 20px;"><input type=hidden name="totallica" id="totallica" /><span>Rp </span><span id=total style="font-size: 45px;"></span>,00</td></tr>'+
                        '<tr><td>TUNAI:</td><td style="font-size: 20px;"><input type=text name="tunai" id="tunai" onkeyup="FormNum(this);" onblur="hitung_kembalian();" /></td></tr>'+
...
function cetak_struk(id_penjualan) {
    ...
    var x = screen.width/2 - dWidth/2;
    var y = screen.height/2 - dHeight/2;
    window.open('pelayanan/manage_penjualan/cetak/?id='+id_penjualan, 'Penjualan Cetak', 'width='+dWidth+', height='+dHeight+', left='+x+',top='+y);
}

function hitung_kembalian() {
    var pembulatan = parseInt(currencyToNumber($('#total').html()));
    var pembayaran = parseInt(currencyToNumber($('#tunai').val()));
    var kembalian  = Math.ceil(pembayaran - pembulatan);
