...
<div class="kegiatan">
        ...
        $(function() {
            ...
            $('#formigd').submit(function(){
                ...
                if($('input[name=id_dpjp]').val() == ''){
                    custom_message('Peringatan','D.P.J.P. tidak boleh kosong !','#dpjp');
                    return false; 
                }
                ...
                if(!request) {
                    request = $.ajax({
                    type : 'POST',
                    url: '<?= base_url("pelayanan/igd_save") ?>',
                    data: $(this).serialize(),
                    cache: false,
                    dataType: 'json',
                    success: function(msg){
                        ...
                        if (msg.status == true){
                           ...
                           $('input[name=id_pelayanan_kunjungan]').val(msg.id_pelayanan);
                            ...
                            custom_message('Peringatan',msg.status);
                        }
                        ...
                        success: function(data) {
                            if(data.length !== 0){
                                ...
                                load_data_pelayanan(data.no_daftar);
                                 ...
                                 custom_message('Peringatan!', 'Pasien tidak mendaftar pada layanan IGD', '#no_barcode');
                                $('#no').val('');
                            }
    ...
    function reset_all_data(){
        ...
        $('#ku, #tensi, #nadi, #suhu, #nafas, #bb, #no_polis, #asuransi').val('');
    ...
    function load_data_pelayanan(no_kunjungan){
            $.ajax({
                url: '<?= base_url('pelayanan/load_data_pelayanan_kunjungan') ?>/'+no_kunjungan+'/igd',
                cache: false,
                dataType : 'json',
                success: function(data) {
                    if (!jQuery.isEmptyObject(data)) {
                        ...
                        $('#lanjut').val(data.rencana_tindak_lanjut);
        ...
        function load_data_diagnosis(no_kunjungan) {
            $.ajax({
                url: '<?= base_url('pelayanan/load_data_diagnosis') ?>/'+no_kunjungan+'/igd',
                cache: false,
                success: function(data) {
        ...
        function load_data_tindakan(no_kunjungan) {
            $.ajax({
                url: '<?= base_url('pelayanan/load_data_tindakan') ?>/'+no_kunjungan+'/igd',
                cache: false,
                success: function(data) {
        ...
        function add(i) {
            ...
            var str = '<tr class="row_diag">'+
                    '<td><input type="text" name="waktu_diag[]" id=waktu_diag'+i+' value="<?= date('d/m/Y H:i') ?>" /></td>'+
                    '<td><input type="text" name="dok_diag" value="<?= $this->session->userdata('nama') ?>" id=dokter_diag'+i+' /><input type="hidden" value="<?= $this->session->userdata('id_pegawai') ?>" name="dokter_diag[]" id=id_dokter_diag'+i+' /></td>'+
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(obj, function ( index, val) {
                        $('#unit_diag'+i).append('<option value="'+val.id+'">'+val.nama+'</option>');
                    });
                    $('#unit_diag'+i).val('<?= $this->session->userdata("id_unit")?>');
                }
            });
    ...
    function add_tindak(i) {
        str = '<tr class="row_tindak">'+
            ...
            '<td><input type="text" name="nakes_tindak[]" value="<?= $this->session->userdata('nama') ?>" id=nakes_tindak'+i+' /><input type="hidden" value="<?= $this->session->userdata('id_pegawai') ?>" name="id_nakes_tindak[]" id=id_nakes_tindak'+i+' /></td>'+
...
//            '<td><input type="text" name="icd_tindak[]" id=icd_tindak'+i+' /></td>'+
//            '<td><span class="ic link_button" id=ic'+i+'>I.C.</span></td>'+
            '<td class=aksi align=center><a class="deletion" onClick="eliminate(this)"></a></td></tr>';  
        ...
        $('#nakes_tindak'+i).autocomplete("<?= base_url('inv_autocomplete/load_data_pegawai') ?>",
            ...
            function(event,data,formated){
                ...
                $('#id_layanan'+i).val(data.id_layanan);
    ...
    function reset_all(){
        $('#loaddata').empty();
        $('#loaddata').load('<?= base_url("pelayanan/igd") ?>');
    }
    ...
    function cetak_fisio(){
        ...
        if(id !== ''){
            window.open('<?= base_url("laboratorium/cetak_sp_pemeriksaan_fisioterapi") ?>/'+id, 'mywindow', 'width=820px, height=300px, scrollable=1, resizable=0');
        }else{
            custom_message('Alert !', 'Belum memasukkan data pasien', '#no');
        }
    }
        ...
        <div id="tabs-utama">
            ...
            <div id="tabs-utama-2">
                <table width="100%" cellpadding="0" cellspacing="0"><tr valign="top"><td width="50%">
                    <table width="100%" class="inputan">
                        ...
                        <tr><td>Nama Asuransi:</td><td><?= form_input('',isset($data->asuransi)?$data->asuransi:NULL,'id=asuransi class=input-text size=40')?></td></tr>
                        <tr><td>No. Polish Asuransi:</td><td><?= form_input('no_polis',isset($data->no_polis)?$data->no_polis:NULL,'id=no_polis class=input-text size=40')?></td></tr>
                        <tr><td>Dokter Penanggung Jwb:</td><td><?= form_input('',isset($data->nama)?$data->nama:$this->session->userdata('nama'),'id=dpjp class=input-text size=40')?></td></tr>
                        <tr valign="top"><td>Anamnesis:</td><td><?= form_textarea('anamnesis',isset($data->anamnesis)?$data->anamnesis:'',' class=standar id=anamnesis row=3')?></td>
                        </tr>
                    </table>
                    </td><td width="50%">
                    <table width="100%" class="inputan">
                        <tr><td style="width: 150px;">Tensi:</td><td><?= form_input('tensi',isset($data->p_tensi)?$data->p_tensi:NULL,'id=tensi size=15')?> mm/Hg</td></tr>
            ...
            <div id="tabs-utama-3">
                <div id="tabs">
                    ...
                    <div id="tabs-1">
                        <table width="100%" class="inputan">
                            <?= form_hidden('id_dokter_lab',$this->session->userdata('id_pegawai')) ?>
                            <?= form_hidden('id_analis_lab') ?>
                            <?= form_hidden('id_layanan_lab') ?>
                            <?= form_hidden('id_satuan_lab') ?>
                            <?php $ket = array(''=>'Pilih...','L'=>'Low','N'=>'Netral', 'H'=>'High' ) ?>
                            <tr><td style="width: 150px;">Nama Dokter:</td><td><?= form_input('',$this->session->userdata('nama'),'id=dokter_lab size=40')?></td></tr>
                    ...
                    <div id="tabs-2">
                        <table width="100%" class="inputan">
                                ...
                                <tr><td></td><td><?= form_button(NULL, 'Tambah (F7)', 'id=bt_rad') ?>
                                <?= form_button('','Cetak S.P. Radiologi', 'class=print onclick=cetak_radio()') ?> </td></tr>
                        </table>
                    ...
                    <?php if (isset($tindakan)) { 
                        foreach ($tindakan as $key => $rows) {
                            ...
                            <tr>
                                ...
                                <td><?= $rows->nama_unit ?></td>
                                <td><?= $rows->tindakan ?></td>
<!--                                <td align="center"><?= $rows->kode_icdixcm ?></td>
                                <td align="center">-</td>-->
                                <td align="center">-</td>
                            </tr>    
                    <?php } 
            ...
            <table width="100%">
                <tr valign="top"><td>Rencana Tindak Lanjut:<br/><br/><?= form_textarea('lanjut',isset($data->rencana_tindak_lanjut)?$data->rencana_tindak_lanjut:'','id=lanjut row=3')?></td></tr>
            </table>
            <?= form_submit('','Simpan','id=simpan_igd') ?>
