...
function dialog_rekening() {
    var str = '<div class=data-input id=dialogx><form action="" id=form_rekening>'+
        '<table width="100%" class=inputan>'+
            '<tr><td>Kode.:</td><td><?= form_input('kode_rek',get_last_id('rekening', 'id'),'id=kode_rek') ?><input type=hidden name=kode_rek_id id=kode_rek_id /></td></tr>'+
...
<style>
    .list-data td { text-align: left; text-transform: capitalize; }
/*    .list-data th:last-child, .list-data td:last-child { display: none; }*/
</style>
<div class="data-list">
    <div id="dialog_form"></div>
    <table class="list-data-advance list-data" width="100%" id="example-advanced">
        <tr>
            <th width="25%">Kode</th>
            <th width="45%">Nama</th>
            <th width="25%">#</th>
        ...
        <?php 
            ...
            foreach ($sub_rekening as $r2 => $rows) { ?>
                <tr data-tt-id='<?= $r1 ?>-<?= $r2 ?>' data-tt-parent-id='<?= $r1 ?>' class="even">
                    ...
                    <td style="padding-left: 20px;"><?= $rows->nama ?></td>
                    <td style="padding-left: 20px;"><?= anchor('akuntansi/delete_subrekening/'.$rows->id, 'Hapus', 'class=delete') ?> <?= anchor('akuntansi/tambah_sub_subrekening/'.$rows->id, 'Tambah Sub', 'class=add_subsubrekening id="'.$rows->id.'#'.$rows->nama.'#'.$data->nama.'#'.$data->id.'" title="'.$rows->id.'#'.$rows->nama.'#'.$data->nama.'#'.$data->id.'"') ?></td>
                </tr>
                    <?php 
                    ...
                    foreach ($sub_sub_rekening as $r3 => $rowx) { ?>
                        <tr data-tt-id='<?= $r1 ?>-<?= $r2 ?>-<?= $r3 ?>' data-tt-parent-id='<?= $r1 ?>-<?= $r2 ?>' class="even">
                            ...
                            <td style="padding-left: 40px;"><?= $rowx->nama ?></td>
                            <td style="padding-left: 40px;"><?= anchor('akuntansi/delete_subsubrekening/'.$rowx->id, 'Hapus', 'class=delete') ?> <?= anchor('akuntansi/tambah_sub_sub_subrekening/'.$rowx->id, 'Tambah Sub', 'class=add_subsubsubrekening id="'.$rowx->id.'#'.$rowx->nama.'"') ?></td>
                        </tr>
                        <?php
                        ...
                        foreach ($sub_sub_sub_rekening as $r4 => $rowy) { ?>
                            <tr data-tt-id='<?= $r1 ?>-<?= $r2 ?>-<?= $r3 ?>-<?= $r4 ?>' data-tt-parent-id='<?= $r1 ?>-<?= $r2 ?>-<?= $r3 ?>' class="even">
                                ...
                                <td style="padding-left: 60px;"><?= $rowy->nama ?></td>
                                <td style="padding-left: 60px;"><span style="cursor: pointer;" onclick="delete_sss(this, <?= $rowy->id ?>)">Hapus</span> <?= anchor('', 'Tambah Sub', 'class=add_subsubsubsubrekening id="'.$rowy->id_sub_sub_sub_rekening.'#'.$rowy->id_sub_sub_rekening.'#'.$rowy->id_sub_rekening.'#'.$rowy->id_rekening.'"') ?></td>
                            </tr>
                        <?php
                            ...
                            foreach ($sub_sub_sub_sub_rekening as $r5 => $rowz) { ?>
                                <tr data-tt-id='<?= $r1 ?>-<?= $r2 ?>-<?= $r3 ?>-<?= $r4 ?>-<?= $r5 ?>' data-tt-parent-id='<?= $r1 ?>-<?= $r2 ?>-<?= $r3 ?>-<?= $r4 ?>' class="even">
                                    ...
                                    <td style="padding-left: 80px;"><?= $rowz->sub_sub_sub_sub_rekening ?></td>
                                    <td style="padding-left: 80px;"><span style="cursor: pointer;" onclick="delete_ssss(this, '<?= $rowz->id ?>')">Hapus</span></td>
                                </tr>
                            <?php
